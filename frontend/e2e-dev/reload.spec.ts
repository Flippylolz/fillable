...
async function replaceFile(path: string, content: string) {
  const temporary = path + ".verification-write";
  await writeFile(temporary, content);
  await rename(temporary, path);
}

test("source reload reaches the open browser and API", async ({
  page,
  request,
}) => {
  ...
  try {
    ...
    await expect
      .poll(
        async () => {
          try {
            return (await request.get("/api/development-probe")).status();
          } catch {
            return 0;
          }
        },
      ...
      .toBe(200);
    expect(await (await request.get("/api/development-probe")).json()).toEqual({
      probe: "updated",
    });
  } finally {
    await replaceFile(frontend, originalFrontend);
    await replaceFile(backend, originalBackend);
  }
