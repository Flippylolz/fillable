...
test("multiline and long Unicode values synchronize both ways and reopen through the Python exporter", async ({ page }) => {
  const errors: string[] = [];
  page.on("pageerror", error => errors.push(error.message));
  await page.goto("/prototype.html");
  const clients = page.getByRole("textbox", { name: "Значення поля: ПІБ клієнта", exact: true });
  const first = "Ґанна Їжак\nЄва\t🙂", direct = "Ілля Ґудзик\nЇжак\t🙂";
  await clients.first().fill(first);
  await expect(clients.last()).toHaveValue(first);
  await page.getByRole("button", { name: "Перейти до поля: ПІБ клієнта", exact: true }).last().click();
  await page.keyboard.insertText(direct);
  await expect(clients.first()).toHaveValue(direct);
  await expect(clients.last()).toHaveValue(direct);
  await page.getByRole("button", { name: "Скасувати", exact: true }).click();
  await expect(clients.first()).toHaveValue(first);
  await page.getByRole("button", { name: "Повторити", exact: true }).click();
  const organization = page.getByRole("textbox", { name: "Значення поля: Організація", exact: true });
  const invalid = "🙂".repeat(65537);
  await organization.fill(invalid);
  await expect(organization).toHaveValue(invalid);
  await expect(organization).toHaveAttribute("aria-invalid", "true");
  await expect(page.getByRole("button", { name: "Експортувати DOCX", exact: true })).toBeDisabled();
  const long = "Їжак і Ґудзик 🙂 — контактний зв’язок. ".repeat(100);
  await organization.fill(long);
  await expect(organization).not.toHaveAttribute("aria-invalid");
  await page.getByRole("button", { name: "English", exact: true }).click();
  await expect(page.getByRole("textbox", { name: "Field value: Організація", exact: true })).toHaveValue(long);
  const download = page.waitForEvent("download");
  await page.getByRole("button", { name: "Export DOCX", exact: true }).click();
  await (await download).saveAs("/tmp/fillable-dev-results/multiline.docx");
  await page.getByRole("button", { name: "Reopen exported DOCX", exact: true }).click();
  const reopened = page.getByRole("textbox", { name: "Field value: ПІБ клієнта", exact: true });
  await expect(reopened.first()).toHaveValue(direct);
  await expect(reopened.last()).toHaveValue(direct);
  await expect(page.getByRole("textbox", { name: "Field value: Організація", exact: true })).toHaveValue(long);
  expect(errors).toEqual([]);
});
