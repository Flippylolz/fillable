...
import { useSourceLayout, type SourcePresentation } from "../editor/SourceLayout";

export function HistoricalPreview({ document, presentation }: { document: object; presentation?: SourcePresentation }) {
  const layout = useSourceLayout(presentation);
  ...
  useEffect(() => {
    const view = mountEditor(host.current!, initial.current, { presentation, canEdit: () => false, onChange: () => {},
  ...
  return <div className="history-preview">
    ...
    <div className="document-canvas" ref={host} data-layout={layout.scope} />
