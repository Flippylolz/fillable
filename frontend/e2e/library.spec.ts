...
test("library uploads both kinds, retries safely, and keeps drafts across a language change", async ({ page }, testInfo) => {
  ...
  await expect(card.getByRole("button", { name: "Move to trash", exact: true })).toBeFocused();
  await card.getByRole("button", { name: "Move to trash", exact: true }).click();
  ...
  expect(afterDelete.used_bytes).toBe(beforeDelete.used_bytes);
  await openNavigation(page);
  await page.getByRole("link", { name: "Trash", exact: true }).click();
  const trashCard = page.getByRole("article", { name: documentTitle });
  await trashCard.getByRole("button", { name: "Restore", exact: true }).click();
  ...
  await page.getByRole("link", { name: "Trash", exact: true }).click();
  page.once("dialog", dialog => dialog.accept());
  await trashCard.getByRole("button", { name: "Delete permanently", exact: true }).click();
  await expect(trashCard).toHaveCount(0);
  expect((await (await page.request.get("/api/storage/usage")).json()).used_bytes).toBe(beforeDelete.used_bytes - bytes.length);
  await page.reload();
  await openNavigation(page);
  await page.getByRole("link", { name: "My documents", exact: true }).click();
  await expect(card).toHaveCount(0);
  ...
  await openNavigation(page);
  await page.getByRole("link", { name: "Шаблони", exact: true }).click();
  await templateCard.getByRole("button", { name: "До кошика", exact: true }).click();
  ...
  await openNavigation(page);
  await page.getByRole("link", { name: "Мої документи", exact: true }).click();
  await copyCard.getByRole("button", { name: "До кошика", exact: true }).click();
  await page.getByRole("button", { name: "Перемістити до кошика", exact: true }).click();
