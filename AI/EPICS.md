...
| E09 | Post-release user-requested improvements and automation | E08 | in_progress: E09.1–E09.24 sections below |
...
Status: ready. User request: a "Save to documents" action that prompts for the new document's name, defaulting to the template name plus the first field's data. Acceptance: a template workspace shows a localized "Зберегти в документи"/"Save to documents" toolbar action and a document workspace does not; activating it opens a small prompt form for the new document's title, prefilled with a default composed from the template's title plus — when at least one field carries a nonempty value — the first such field's value in document order, whitespace-collapsed and bounded so the default never exceeds the existing 160-code-point title limit (without a filled value the default is just the template title); if the template has unsaved content changes the action first performs the ordinary manual save with all existing contracts (editing lease, field validity, composition, revision fence) and a failed save aborts the copy with the existing localized save error, while a clean template copies without creating a redundant revision; creation reuses the existing idempotent, quota-checked template-copy endpoint and owner/CSRF rules, so the created resource is an independent document holding a snapshot of the saved template revision and its field metadata while the source template and its history stay unchanged; success shows a localized confirmation with the created document's title and an action that opens it through the same guarded navigation as opening a library card (explicit discard when the template workspace is dirty again); failures (quota, conflict, rate limit, network) show localized recoverable errors and rotate the idempotency key on definitive failures like the library copy form; the action is disabled while a save/restore/rename is pending and hidden while the history panel is open; the library card's existing "Use template" behavior and naming stay unchanged. Component tests cover the default-title composition (filled/empty/truncation), the save-before-copy and clean-copy paths, error recovery, and the template-only visibility. Both catalogs stay complete and the independent 90% gates hold. Deliver a dedicated PR with required CI, then deploy through Actions while preserving existing services.

## E09.17 — Print the current document from the workspace

Status: ready. User request: a print button that saves the document and tries to print it, suggesting a local save when printing is not accessible. Acceptance: template and document workspaces show a localized "Друк"/"Print" toolbar action; if the working document has unsaved content changes, print first performs the ordinary manual save with all existing contracts, and a failed or refused save (invalid field values, pending composition, lost lease, revision conflict, quota) shows the existing localized save error and starts no print; once the content is saved, the app renders the settled working document — identical to the saved revision — alone in a print-only view at 100% zoom with the retained source presentation and without any application chrome, then invokes the browser's print dialog for exactly that view; if in-page printing is unavailable or fails, a localized suggestion offers saving the file locally through the existing saved-download action instead of failing silently; printing creates no revision beyond that one save, never triggers autosave by itself, and does not alter the document model, review state, selection or draft; the printed output is the browser's rendering of the document flow — approximate pagination, no Word-exact print layout and no server-side PDF conversion, keeping [Product](PRODUCT.md)'s PDF-export deferral — recorded in [Workspace](WORKSPACE.md). Component tests cover the save-then-print sequence, the abort paths, and the fallback suggestion with a mocked print bridge; native dialog behavior itself is verified manually and recorded. Both catalogs stay complete and the independent 90% gates hold. Deliver a dedicated PR with required CI, then deploy through Actions while preserving existing services.
