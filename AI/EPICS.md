...
## E09.6 — Automatic Dependabot updates with automerge

Status: in_review. User request: implement automatic Dependabot updates with automerge. Acceptance under D025: weekly Dependabot version updates cover GitHub Actions workflows, the frontend npm manifest, `infra/` Dockerfiles, and root Docker Compose files, with minor/patch updates grouped per ecosystem and major updates standalone; a dedicated workflow enables squash auto-merge only on `dependabot[bot]` pull requests, armed on the exact head commit; every automatic merge still passes the strict up-to-date `ci-required` gate and never deploys. Backend Python dependencies stay on the manual pip-tools procedure because the hash-pinned `requirements.lock` is not a Dependabot-recognized manifest; that limitation is recorded in D025 and [CI and deployment](CI_CD.md). Deliver a dedicated PR with required CI and unchanged independent 90% gates, then verify Dependabot's first update run on the repository.
