...
Local Docker verification: frontend ESLint, catalog/copy checks and the TypeScript production build passed in the pinned image; the full Vitest suite passed 293 tests across 47 files with raw source-bound coverage of 1721/1774 lines (97.01%) and 1959/2108 branches (92.93%), accepted by the independent gate. Tests cover the print-only frame content (layout rules carried over, page-break markers stripped, no application chrome), unavailable-print false paths, the clean print without a redundant revision, save-then-print against the newly saved revision, the abort on a failed pre-save, and the localized local-save suggestion beside the existing download action. The native print dialog itself is platform UI outside component/browser automation; the click-to-print path with the real canvas was exercised in headless Chromium against the development stack, and the dialog presentation remains a documented manual check. Backend source is unchanged; required CI reruns all suites and gates.
