...
Planning task P04: record the user's Fillable redesign plan in one documentation PR (`task/p04-redesign-plan`). Acceptance: the supplied "Tactile Document Gallery" plan (sidebar/top-bar/tabs/gallery composition, locked palette, Sora/Manrope, interaction and overflow requirements, scope boundary, and the Lovable design-prototype preview link) is recorded verbatim in [Redesign plan](REDESIGN_PLAN.md) with its provenance and repository-context discrepancy noted. Recording accepts no implementation scope: any future implementation needs its own task reconciled with the existing library/workspace pages and accepted decisions, including D021 Ukrainian-default localization. Validate Markdown links and consistency; no application coverage is claimed.
...
Status: done, delivered through PR #88 (squash `a3124e768f912e85d2fbb420e757347a76773500`). The single sequential `checks` job made every PR wait for the sum of image builds, lints, both test suites, browser flows, development verification and contract tests, with a 30-minute ceiling. Acceptance: split static checks and tests into independent parallel jobs (`lint`, `backend-tests`, `frontend-tests`, `browser`, `development`, `contracts`) beside the unchanged `upgrade-checks`; keep the independent raw 90% line/branch gates, source provenance binding, both real unimported-source negative probes and fail-closed `ci-required` aggregation over all seven job results; update the deployment gate and its contract tests to the exact new job set and build the two release images concurrently; keep the E09.6 deploy-main dispatcher, E09.7 Dependabot automerge and their contract tests required (in the `contracts` job) and unchanged; keep branch protection on the single `ci-required` context and update AI documentation. No threshold, exclusion, or required check may be relaxed.

## E09.9 — Restore the deploy-dispatch contract in the split CI

Status: done, delivered through PR #97 (squash `6674d54265c925ad204a1a2a70463b155da3acd8`). E09.8's CI split dropped `scripts/test_deploy_dispatch_contract.py` from required CI while the script and the E09.6 dispatcher stayed in use. Acceptance: the dispatch-dispatcher contract test runs again inside the required `contracts` job against the checked-out source, keeping the deploy-on-merge behavior pinned. Deliver a dedicated PR with required CI; the merge itself then deploys automatically through the E09.6 dispatcher.

## E09.10 — Dependabot updates for backend Python dependencies

Status: done, delivered through PR #98 (squash `67e185204778a6cfa7fb865e50bf65169f72a273`). Follow-up to E09.7 and the user request to update Python dependencies the same way as the sibling WEF/estate_test repository: cover backend Python dependencies through the Dependabot `pip` ecosystem. Acceptance: the pip-compile lockfile lives at the Dependabot-recognized path `backend/requirements.txt` (renamed from `requirements.lock` with the Docker install path updated and the header normalized by a real pip-tools 7.5.3 recompile in the digest-pinned base image); `dependabot.yml` gains a grouped `pip` entry for `/backend`; hash-pinned `--require-hashes` installation is unchanged and remains enforced by the Docker build; D025 and [CI and deployment](CI_CD.md) no longer record a Python exclusion; both ecosystems' PRs flow through the existing squash automerge and `ci-required` gate. Configuration/rename only — no application source changes, so coverage is unchanged; the backend test image build validates the renamed lock locally, and required CI reruns all suites and gates.

## E09.11 — Dependabot automerge branch reconciliation

Status: done, delivered through PR #103 (squash `af0f9f45b37c5af59a3ffe3ad20e0a766ad3de6f`) with the follow-up hardenings #104, #105, #108, and #109 merged in turn. The first Dependabot wave showed the automerge workflow arming every pull request but none of them merging: `main` protection requires branches to be up to date, Dependabot rebases only on conflict, and every merge to `main` re-staled the remaining pull requests, which then sat behind with stale green checks indefinitely. Acceptance: `.github/workflows/dependabot-automerge.yml` also updates a behind Dependabot branch through the update-branch API bound to the expected head SHA (letting the resulting synchronize event arm the new head) and reconciles all open Dependabot pull requests after every push to `main` plus a twice-hourly offset schedule fallback — updating behind branches and arming squash auto-merge on pull requests that are neither armed nor carrying failed/cancelled checks, so remaining updates follow each merge with one required-CI cycle; the trust contract is unchanged (`pull_request_target` from the trusted base, no checkout, repository + author guards, only `contents: write`/`pull-requests: write`, no deployment dispatch, merges still gated by strict up-to-date `ci-required`); `scripts/test_dependabot_automerge_contract.py` pins the contract in the required `contracts` job; D025 and [CI and deployment](CI_CD.md) record the reconciliation behavior. Workflow/configuration and contract test only — no application source changes, so coverage is unchanged; the required CI run re-runs all suites and gates. Delivered as #103; follow-ups: #104 moved the fallback cron from `*/30` to `13,43 * * * *` after GitHub's scheduler produced no runs at the packed `:00`/`:30` slots (the repository's Actions history records zero `schedule` runs), and the reconciliation then became push-driven on `main` so no scheduled run is required for convergence. A second hardening followed the first push run matching nothing: the Dependabot author filter is applied client-side over all open pull requests (the bot login differs between REST and GraphQL and `--author` is unreliable under `GITHUB_TOKEN`), and behind-ness is computed from the compare API because `mergeStateStatus` caches stale values for minutes after a merge. A third hardening approves the `action_required` pull_request runs that `GITHUB_TOKEN` branch updates create, so the reconciled required CI actually executes without a maintainer waiting; a refused self-approval is reported for manual attention.

## E09.12 — Stabilize the history restore browser check

Renumbered from a provisional E09.10 ID now used by the merged Dependabot Python task. Status: done, delivered through PR #99 (squash `e29aeba99f5da40eef0d01eaff4553de0aeb38ed`). CI run 34259413491 flaked in the required browser job: after accepting the restore confirm, the desktop history check waited only five seconds for the toolbar button that reappears when the panel closes, so a missed dialog or a slow restore surfaced as an unrelated visibility failure and blocked `ci-required` and that cycle's automated deployment (identical browser content passed in the adjacent runs). Acceptance: the restore must be confirmed through the document endpoint before the panel-close assertion runs, keeping every original assertion and producing a direct failure if the restore does not happen. Test-only change; the independent 90% gates, catalogs and deploy behavior are untouched.
## E09.13 — Checkbox fidelity and toggling for form documents

Status: done, delivered through PR #106 (squash `a3ba96b3503e67c8444fbd5139c097dbcb6afe13`) with the follow-up fixes #124 and #125 merged in turn. User-reported on a real Ukrainian form: checkbox graphics render misaligned and cannot be modified. Acceptance: native `w14:checkbox` content controls import as bounded, accessible checkbox nodes whose checked state toggles by direct click or Space on the selection with single-step undo; export rewrites only that control's state value and state glyph while an unchanged document stays byte-identical and malformed controls remain protected as unsupported; a localized selection action toggles one unambiguous ballot-box character (U+2610/U+2612) preserving run formatting, with Wingdings-style private-use codes excluded because run fonts are outside the editing model; inline DrawingML rectangles and VML rectangles are extracted as bounded read-only shapes rendered in the text flow at their anchor position instead of disappearing, and anchored shapes keep their documented paragraph-relative placement; supported structure is recorded in [Workspace](WORKSPACE.md) and [Editor feasibility](EDITOR_FEASIBILITY.md). Both catalogs stay complete and the independent 90% gates hold. Deliver a dedicated PR with required CI, then deploy through Actions while preserving existing services.

2026-09-09: Follow-up user feedback on the delivered graphics work (real TB-09 discharge form): every anchored checkbox rectangle rendered twice because `display()` collected the DrawingML choice and the duplicate VML fallback of one `AlternateContent` drawing independently — the duplicates appeared as three stray boxes before item 1 and as staggered boxes over items 35–37. Shapes now enumerate once per drawing (DrawingML preferred, fallback only when the choice yields no bounded shape), fixing the duplicates. The same feedback asked for the drawn boxes to be selectable: rectangles with an explicit fill toggle like checkboxes (click swaps between the form's dark fill and a cleared white box in one undo step, stored as a bounded per-run `shapes` override in the editor model), export rewrites only that rectangle's fill in the choice and fallback, and unchanged or empty-override saves stay byte-identical. Anchored vertical placement remains the documented paragraph-relative approximation, and exact Word pagination stays unclaimed. 2026-09-10: Follow-up on the same form: the drawn boxes hung below their labels because the editor wraps and leads lines differently from Word. Checkbox-sized anchored rectangles are now re-anchored after layout onto the rendered text line nearest the offset Word chose (bounded, presentation-only, larger frames keep exact offsets), measured level with their labels on the reproduced document.

## E09.14 — Open a resource by clicking its library card

Status: done, delivered through PR #118. User feedback on the library card: the card itself does nothing when clicked and the small "Відкрити" link is the only way in — reported as not clickable and useless. Acceptance: clicking anywhere on a library card (template or document) that is not another control opens that resource in the workspace through the card title link stretched over the card (middle/modified clicks keep native new-tab behavior, the blocked/disabled state swallows the click); the standalone "Відкрити" link is removed and its catalog entries drop from both locales; download, use-template, processing retry, and delete controls keep working above the card link, and deletion-pending cards do not open; component tests cover the new click behavior and existing open flows use the card link. Both catalogs stay complete and the independent 90% gates hold. Deliver a dedicated PR with required CI, then deploy through Actions while preserving existing services.

## E09.15 — Visible page breaks in the editor

Status: done, delivered through PR #119. User feedback: the document does not show where one page ends and the next begins. Acceptance: the mounted workspace and historical editors show approximate page boundaries derived from the source page geometry already delivered for layout (page height/margins), as dashed, localized "Сторінка N"/"Page N" markers between blocks that begin a new page; markers are purely visual ProseMirror decorations — never in the document model, review metadata, save payload or exports — recompute after edits, zoom and layout/font changes, honor the zoom scale, and stay hidden from assistive technology; documents without page geometry show no markers; breaks fall on block boundaries only, exact Word pagination stays unclaimed and recorded as unproved in [Editor feasibility](EDITOR_FEASIBILITY.md) and [Workspace](WORKSPACE.md); both catalogs stay complete and the independent 90% gates hold. Deliver a dedicated PR with required CI, then deploy through Actions while preserving existing services.

## E09.16 — Save a template to documents from the workspace

Status: done, delivered through PR #128 (squash `be2c5819fb51703119696d6645925e1640213ae1`). User request: a "Save to documents" action that prompts for the new document's name, defaulting to the template name plus the first field's data. Acceptance: a template workspace shows a localized "Зберегти в документи"/"Save to documents" toolbar action and a document workspace does not; activating it opens a small prompt form for the new document's title, prefilled with a default composed from the template's title plus — when at least one field carries a nonempty value — the first such field's value in document order, whitespace-collapsed and bounded so the default never exceeds the existing 160-code-point title limit (without a filled value the default is just the template title); if the template has unsaved content changes the action first performs the ordinary manual save with all existing contracts (editing lease, field validity, composition, revision fence) and a failed save aborts the copy with the existing localized save error, while a clean template copies without creating a redundant revision; creation reuses the existing idempotent, quota-checked template-copy endpoint and owner/CSRF rules, so the created resource is an independent document holding a snapshot of the saved template revision and its field metadata while the source template and its history stay unchanged; success shows a localized confirmation with the created document's title and an action that opens it through the same guarded navigation as opening a library card (explicit discard when the template workspace is dirty again); failures (quota, conflict, rate limit, network) show localized recoverable errors and rotate the idempotency key on definitive failures like the library copy form; the action is disabled while a save/restore/rename is pending and hidden while the history panel is open; the library card's existing "Use template" behavior and naming stay unchanged. Component tests cover the default-title composition (filled/empty/truncation), the save-before-copy and clean-copy paths, error recovery, and the template-only visibility. Both catalogs stay complete and the independent 90% gates hold. Deliver a dedicated PR with required CI, then deploy through Actions while preserving existing services.

Local Docker verification: frontend ESLint, catalog/copy checks and the TypeScript production build passed in the pinned image; the full Vitest suite passed 285 tests across 45 files with raw source-bound coverage of 1686/1739 lines (96.95%) and 1925/2070 branches (93.00%), accepted by the independent gate. Component tests cover the composed default (including placeholder-text field data and code-point clipping), template-only visibility, the clean-copy path without a redundant revision, the dirty save-then-copy path against the newly saved revision, the abort on a failed pre-save, and idempotency-key rotation after a definitive copy failure. Backend source is unchanged from `main`; required CI reruns all suites and gates.

## E09.17 — Print the current document from the workspace

Status: done, delivered through PR #129 (squash `fb75d17f3bcd9fbe0fa4e4e4330a859051a5ede1`). User request: a print button that saves the document and tries to print it, suggesting a local save when printing is not accessible. Acceptance: template and document workspaces show a localized "Друк"/"Print" toolbar action; if the working document has unsaved content changes, print first performs the ordinary manual save with all existing contracts, and a failed or refused save (invalid field values, pending composition, lost lease, revision conflict, quota) shows the existing localized save error and starts no print; once the content is saved, the app renders the settled working document — identical to the saved revision — alone in a print-only view at 100% zoom with the retained source presentation and without any application chrome, then invokes the browser's print dialog for exactly that view; if in-page printing is unavailable or fails, a localized suggestion offers saving the file locally through the existing saved-download action instead of failing silently; printing creates no revision beyond that one save, never triggers autosave by itself, and does not alter the document model, review state, selection or draft; the printed output is the browser's rendering of the document flow — approximate pagination, no Word-exact print layout and no server-side PDF conversion, keeping [Product](PRODUCT.md)'s PDF-export deferral — recorded in [Workspace](WORKSPACE.md). Component tests cover the save-then-print sequence, the abort paths, and the fallback suggestion with a mocked print bridge; native dialog behavior itself is verified manually and recorded. Both catalogs stay complete and the independent 90% gates hold. Deliver a dedicated PR with required CI, then deploy through Actions while preserving existing services.

Local Docker verification: frontend ESLint, catalog/copy checks and the TypeScript production build passed in the pinned image; the full Vitest suite passed 293 tests across 47 files with raw source-bound coverage of 1721/1774 lines (97.01%) and 1959/2108 branches (92.93%), accepted by the independent gate. Tests cover the print-only frame content (layout rules carried over, page-break markers stripped, no application chrome), unavailable-print false paths, the clean print without a redundant revision, save-then-print against the newly saved revision, the abort on a failed pre-save, and the localized local-save suggestion beside the existing download action. The click-to-print path with the real canvas is additionally exercised by a new `e2e/print.spec.ts` browser flow against a production-style stack on desktop and mobile viewports: it uploads a document, opens the workspace, prints, asserts the print-only frame appears without the fallback suggestion and without a new revision, and screenshots the workspace; the native dialog presentation itself remains platform UI outside automation. Backend source is unchanged; required CI reruns all suites and gates.

## E09.18 — Compact field cards in the values sidebar

Status: done, delivered through PR #130 (squash `e018d6d3286fcb03e13bb70ccba200878d807e3c`). User feedback: the selected fields in the template sidebar look too massive and only a few fit on screen at once. Acceptance: the per-field cards in the values sidebar are restyled for density — tighter spacing and typography, the type marker and per-field actions occupy visibly less room, multiline inputs start small and grow with their content, and cards separate primarily by border instead of large margins — while no information or control is removed: labels, typed text/number/date inputs, validation and inconsistent-occurrence messages, focus/remove actions, position navigation and the active-field marker all remain, keyboard reachable and without hover-only affordances; a typical one-line field card becomes roughly half its previous height so substantially more fields are fully visible in the same sidebar at a 1280×800 desktop viewport, verified by inspection against the synthetic corpus; nothing overflows horizontally in the fixed sidebar column on desktop or a 390px mobile viewport, and hit areas stay usable. Component tests keep covering navigation, validation and actions against the updated markup. Both catalogs stay complete (copy is unchanged or complete) and the independent 90% gates hold. Deliver a dedicated PR with required CI, then deploy through Actions while preserving existing services.

Local Docker verification: frontend ESLint, catalog/copy checks and the TypeScript production build passed in the pinned image; the full Vitest suite passed 275 tests across 43 files with raw source-bound coverage of 1633/1685 lines (96.91%) and 1864/1998 branches (93.29%), accepted by the independent gate. Density was measured in headless Chromium against a real workspace on the synthetic corpus at a 1280×800 desktop viewport: a typical one-line field card dropped from 255px to 129px of height (50.6% of the previous size) and the number of cards fully visible inside the 640px sidebar scrollport grew from 1 to 3. Component tests pin the new compact structure — content-sized multiline inputs, an inline header row for the type and active markers, and a single-row navigation header — while the existing navigation, validation, conflict and action coverage carries over unchanged. No backend, catalog-copy, or other files changed.

## E09.19 — Fill mode with a small live preview

Status: done, delivered through PR #131 (squash `797221d337cc937d7017b1bfa1c6f2fffd1d14c7`). User request: switch between (1) seeing the document and selecting fields and (2) a filling-focused view of the already-selected fields with a small preview. Acceptance: the workspace gains a localized two-mode view switch — document mode (the default; the current editor canvas, tools and sidebar) and fill mode (a filling form with a small live preview); fill mode lists the working fields — exactly the occurrences the sidebar already lists, so dismissed entries and unaccepted proposals stay out and review remains a document-mode activity — in document order, each entry showing a readable label, its type, the same typed input and validation as the sidebar, its inconsistent-occurrence note, per-entry focus/remove, and participating in the same active-field selection; fill-mode edits go through the same editor transactions, so two-way consistency, the single undo/redo history, the local revision counter, autosave, lease/read-only enforcement and draft preservation across language changes are all unchanged, with undo/redo still available in fill mode while selection-only tools (create field, boxed date, glyph checkbox) remain document-mode features; a small, non-interactive preview of the live working document sits beside the form (below it on mobile), reuses the retained source presentation, follows edits with a bounded refresh, is never editable and never saves, and exact pagination stays unclaimed as for every rendering; the mode switch is disabled while the history panel is open, review-stale and access banners stay visible in both modes, switching modes preserves the draft, selection and undo stack, and the chosen mode is a display preference of the mounted workspace (like zoom), not an account setting; an empty field list shows a localized empty state pointing back to document mode. Component tests cover the switch, form propagation, read-only behavior, mode persistence within the mount and preview bounds. Both catalogs stay complete and the independent 90% gates hold. Deliver a dedicated PR with required CI, then deploy through Actions while preserving existing services.

Local Docker verification: frontend ESLint, catalog/copy checks and the TypeScript production build passed in the pinned image; the full Vitest suite passed 302 tests across 48 files with raw source-bound coverage of 1750/1806 lines (96.90%) and 1986/2141 branches (92.76%), accepted by the independent gate. New tests cover the document-order fill form with typed inputs and inline markers, propagation through the same editor transactions (linked occurrences stay consistent, single undo/redo history, preview excluded from it), the non-interactive live preview clone (bounded 700 ms refresh, page-break markers stripped, canvas kept mounted but visually hidden with breaks recomputed on return), read-only enforcement, the empty state, and the toolbar switch (aria-pressed pair, disabled while the history panel is open, mode preserved within the mount). The typed entry control is shared between the sidebar cards and the fill form so the two cannot diverge.

## E09.20 — Track host tool versions in the repository

Status: done, delivered through PR #127 (squash `a87c228cb93b823b65fc0702e900bbb67d45f3a5`). The repository root carried an untracked local `.tool-versions` (`python 3.13.2`), so version-manager users (asdf/mise) had a working host `python3` only in their original checkout and not in fresh clones or additional worktrees, which broke convenience commands run outside Docker. Acceptance: the existing `.tool-versions` is tracked at the repository root so every checkout and worktree provisions the same host Python version; [Local development](LOCAL_DEVELOPMENT.md) records that host tools remain optional and that Docker stays the standard and sufficient environment, with host `python3` used only for ad-hoc convenience scripts. No application source, generated artifacts, or lockfiles change; coverage and required checks are unchanged. Delivered through its own PR based on the E09.16–E09.19 planning branch because both touch this ledger; no runtime behavior changes.

## E09.24 — Apply the gallery design system to every remaining surface

Status: done, delivered through PR #137 (squash `14fa9428846662dad7a984fb5de3e72b0612e698`). Outcome: after the gallery redesign, the remaining old-palette styling is swept into the semantic tokens: the sign-in page (card, inputs, feedback, header) is restyled in the gallery card language, and the workspace toolbar/settings/history/copy, editor field sidebar, review cards, fill mode, profile cards, and success/warning/danger states use the tokens (derived success/warning tokens added alongside the locked palette; the version badge keeps its exact D022 colors). No layout or contract changes; both catalogs and the coverage gates hold, and the login/profile/workspace/library pages are visually verified on desktop and mobile.

## E09.23 — Sidebar-only library switching and shell top-margin fix

Status: done, delivered through PR #136 (squash `8de25a29127f3ade2786af35cb10d65206443648`). Outcome: after user feedback on the deployed gallery, the in-page template/document tab list is removed — the sidebar sections (Мої документи, Шаблони) are the only gallery switcher and mark the active gallery with `aria-current` — and the session wrapper no longer adds the sign-in page's vertical margin above the full-viewport shell. Sort, search, upload, and all card actions are unchanged. Acceptance: both catalogs and the coverage gates hold, browser suites pass on desktop and mobile.

## E09.22 — Tactile Document Gallery shell and library redesign

Status: done, delivered through PR #135 (squash `23c0979c40d54eb51b453292bc9e896a10773623`). Outcome: the user's [Redesign plan](REDESIGN_PLAN.md) composition (recorded as planning task P04) is applied to the authenticated application: a responsive collapsible sidebar rail with section links, the storage meter and sign-out; a library top bar with client-side search and the prominent DOCX upload action; tab and sort controls above tactile gallery cards; the locked palette, self-hosted Sora/Manrope tokens and reduced-motion handling in the global styles. Search and sorting are client-side over the existing cursor API; the upload form, idempotency, quota and deletion contracts are unchanged. Workspace, profile and login keep their layouts and inherit the tokens; the version badge keeps its exact D022 styling. Acceptance: both catalogs complete through the required checks, unit and browser suites at the independent gates, desktop/mobile verification on a live stack.

## E09.21 — Keep the selection visible and suggest the field label when creating fields

Status: done, delivered through PR #132 (squash `19389c88349a4c82826bb853c7ab690da86d123c`). User feedback on manual field creation: clicking the label input hides the document selection, so the text being named becomes invisible, and the name should default to the preceding statement instead of empty. Acceptance: while the naming input holds focus, the chosen document range stays visibly highlighted through a presentation-only ProseMirror decoration that never enters the document model, review state, save payload or undo history; it clears itself when the document selection changes or the field is created and works identically on read-only selections; when a valid single-paragraph selection is made, the naming input prefills with the nearest preceding text of that paragraph — whitespace-collapsed, stripped of a short leading numbering marker such as "3. " or "a)", clipped to the label limit and validity-checked — while a user-typed name is never clobbered by later selections and everything remains fully editable; selections inside existing fields suggest nothing. Component tests cover the suggestion rules, the retained-range decoration lifecycle (appear, explicit clear, selection-change clear, no document dirtying) and the unchanged creation flow. Deliver a dedicated PR with required CI, then deploy through Actions while preserving existing services.
...
2026-09-09: E10.1 done — PR #110 (`task/e10-1-password-minimum`, head `59a0a2a`) merged as `f0a07a8` after all eight required jobs (`lint`, `backend-tests`, `frontend-tests`, `browser`, `development`, `contracts`, `upgrade-checks`, `ci-required`) succeeded. Finding F1 was withdrawn on evidence: the repro password `short9char` has ten characters, and the 10–1024 enforcement from E08.6 (PR #77) was already active on the profile endpoint, provisioning, and reset paths — the observed 200/`completed` responses were correct boundary behavior. The PR closed the real gaps: 9-reject/10-accept boundary tests for the provisioning and reset CLI paths (previously missing), the CLI now prints the specific `invalid_password` code, and the profile form validates the minimum inline from both catalogs instead of the native `minLength` bubble. Local validation: backend 437 passed (lines 4227/4268, branches 1338/1384), frontend suites green (lines 1397/1423, branches 1656/1750), ruff/mypy/ESLint/i18n/build clean.

2026-09-09: E10.2 done — PR #111 (`task/e10-2-settings-panel`, head `1568e51`) merged as `485ec75` with every required job successful, including the x64 `browser` job running the new 60+ second panel check. The clean-`main` reproduction required by the epic did not reproduce a collapse: on an isolated dev stack from `dc062be`, the expanded settings panel stayed open 65+ seconds across two real autosaves and multiple 20-second editing-lease renewals, and committed `main` has no closure path for the native `<details>` panel (background cycles only re-render). Root cause recorded in [QA findings](QA_FINDINGS_E10.md): the QA session observed Vite hot updates and React Fast-Refresh remounts from the concurrent agent's live edits to the served working tree, which reset the panel's DOM open state. Hardened anyway: `Workspace` now owns the panel's open state and passes it to `WorkspaceSettings`, so a child remount (the observed collapse mechanism) can no longer collapse it; `frontend/e2e/settings-panel.spec.ts` holds the panel open 60+ seconds with autosave and lease activity on desktop+mobile; a component test covers toggle and remount-reopen. Local validation: frontend 247 tests passed (lines 1397/1423, branches 1651/1745), ESLint/catalog/build clean, e2e spec passed locally on both projects against both the clean-main and fixed stacks.

2026-09-09: E10.3 done — PR #112 (`task/e10-3-undo-redo-state`, head `97a3f5e`) merged as `6c8908b` with every required job successful, including the x64 `browser` job running the new undo/redo spec. The editor adapter's presentation now reports `canUndo`/`canRedo` from the ProseMirror history plugin state (event counts; the library exports no helpers), and the toolbar gates "Скасувати"/"Повторити" on them besides `readOnly`, fixing QA finding F3: disabled on fresh open, enabled by an applicable edit, correct flips across undo/redo, unchanged by autosave, and reset when a history restore remounts the editor. Component test covers the full cycle in both locales; `frontend/e2e/undo-redo.spec.ts` covers fresh-open, edit, autosave, undo/redo and the post-restore state on desktop+mobile. Local validation: frontend 248 tests passed (lines 1399/1425, branches 1662/1758), ESLint/catalog/build clean, e2e verified on an isolated dev stack. One earlier local run had executed against a stale image without the change ("10 passed"); after rebuilding, 11 tests ran and the behavior was re-verified — noted as a local-process lesson, not a product issue.

2026-09-09: E10.4 done — PR #113 (`task/e10-4-login-feedback`, heads `779dd13`→`6f81725`) merged as `e61a4a3` with every required job successful. Empty sign-in submits now validate in the form with catalog messages (`auth.loginRequired`/`auth.passwordRequired`, uk/en, aria-invalid/aria-describedby wiring) instead of native browser bubbles, and the invalid-credentials alert renders inside the sign-in card's reserved feedback row so its appearance cannot shift the card or page — fixing findings F4 and F5. The required mobile `browser` job exposed an enabled-but-dead sign-in button: the submit handler also guards on `childBusy`/`recovering` while the button reflected only `busy`; the button now disables on the same conditions and a successful sign-out resets the recovery flag. Local validation: frontend 249/250 tests passed with independent gates, ESLint/catalog/build clean, auth e2e verified on dev and production-style stacks with desktop/mobile uk/en screenshots; the mobile CI failure was diagnosed via the run's error-context artifact and could not be reproduced locally (10+ runs) before the guard fix.

2026-09-09: E10.5 done — PR #114 (`task/e10-5-audit-trail`) merged as `7d093a0` with every required job successful after one transient `development` flake (an editor-selection poll timing out under runner load) cleared on the failed-job rerun. Original uploads record `original_uploaded` and use-template copies record `document_copied` (owner + authenticated actor, source/target document and version ids, file id; deterministic event ids deduplicate exact replays; rows commit atomically with the resource they describe), and `document_deletion_requested` now records the authenticated actor; `deletion_completed` stays actor-less because the cleanup sweep writes it. The finding's claim that restore rows were missing was corrected: `revision_restored` has existed since E07.2 (PR #64). Both new actions are registered in the diagnostics chronology. Local validation: backend 438 tests passed (lines 4231/4272, branches 1338/1384), ruff/mypy clean.

2026-09-09: E10.6 done — PR #115 (`task/e10-6-invalid-request-parameters`) merged as `6dcd38a` with every required job successful. Schema-validation 422s now carry `{parameter, reason}` (bounded machine-readable name and reason; never submitted values, exception text or internals; request-supplied names bounded to 64 printable characters). The OpenAPI/TypeScript contract was regenerated and is byte-identical (`parameters` was already typed `dict[string|integer]`) — CI's drift check confirms. The frontend interpolates the name via `errors.invalid_request_parameter` in both catalogs where the API names a parameter (profile forms), keeping the generic message as fallback. Local validation: backend gates lines 4233/4276 and branches 1340/1388; frontend 250 tests, lines 1405/1431, branches 1670/1767; ruff/mypy/ESLint/catalog clean.

2026-09-09: E10.7 done — PR #116 (`task/e10-7-manual-pass-favicon`) merged as `35b55e1` with every required job successful, completing epic E10. The pending manual checks are now explicit automated coverage in the required x64 `browser` job (profile GUI including the ten-character minimum message, delete-confirmation cancel/confirm, use-template click-through with the real file input, history panel preview/download/restore with unsaved-work resolution, manual-save failure presentation, English-locale and mobile passes), with per-item results recorded in [QA findings](QA_FINDINGS_E10.md). The favicon fallback (F8) shipped as a 32×32 `favicon.png` and PNG-in-ICO `favicon.ico` rendered from the SVG, declared behind it in `index.html` and asserted by the smoke spec. One new issue found during the pass (an enabled-but-dead sign-in button during the sign-out transition) was fixed in E10.4. Local validation: frontend 250 tests passed (lines 1415/1441, branches 1694/1790), smoke/profile specs verified on a production-style stack on both viewports. All findings F1–F8 are now fixed or dispositioned: F1 withdrawn (miscounted evidence, hardening delivered), F2 environment root cause + hardening, F3–F8 fixed. Every merge deployed through the existing automation.
...
Outcome: fields carry a user-correctable fill-time type — `text`, `number`, or `date` — with deterministic detection proposals and typed sidebar entry, under D026 and the schema contract in [Field schemas](FIELD_SCHEMAS.md). The type is review metadata: documents keep plain-text content controls, and DOCX bytes, export, copies, and restores stay byte-compatible.

Work:

- E11.1: Widen the snapshot/review schemas with a defaulted optional `type` (`text` default, `number`/`date` added), add the deterministic classifier from value shapes and Ukrainian/English label keywords, thread proposals through detection, attach, configure, and accept, render typed sidebar inputs (decimal text input for numbers preserving exact user text; datepicker reading day-first/ISO values and writing canonical `ДД.ММ.РРРР`), extend local value checks (`invalid_number`, `invalid_date`) through the existing issue/save-blocking contract, keep the server treating values as bounded text, regenerate the OpenAPI/TypeScript contract and the editor prototype fixture, and cover schemas, classifier, detectors, review transactions, sidebar behavior, and both catalogs.

Acceptance:

- Detection proposes types deterministically with `text` as the fallback; document numbers and contact/postal labels stay text, and every proposal remains changeable in review before and after acceptance.
- Type changes are undoable review-attribute steps that never alter document content or saved bytes; old snapshots and saved reviews without types keep loading as text.
...
2026-09-11: E09.16–E09.21 completion record. All six tasks merged through individual protected PRs with every required job successful, each merge deploying automatically through the E09.6 gated release: PR #128 `be2c581` (E09.16 save-to-documents), PR #129 `fb75d17` (E09.17 print), PR #130 `e018d6d` (E09.18 compact sidebar), PR #131 `797221d` (E09.19 fill mode), PR #127 `a87c228` (E09.20 tool-versions), and PR #132 `19389c8` (E09.21 selection label suggestions). E09.17's first CI round failed only the new print spec because its browser account was missing in the development job's production phase; the provisioning fix landed in the same PR. Measured frontend coverage on the merged sources stayed above the independent gates in every PR (96.8–97.0% lines, 92.7–93.3% branches as recorded per task above); backend suites reran unchanged. The deployed origin serves the merged bundle through the automatic release verification.

2026-09-12: P04 submitted on branch `task/p04-redesign-plan` to record the user's Fillable redesign plan — the selected "Tactile Document Gallery" composition (collapsible sidebar, search/top bar with DOCX upload, template/document tabs, sorted gallery, locked palette #F7F8FA/#172033/#2563EB/#DCE6F7, Sora/Manrope, 120–180ms transitions, mobile/desktop overflow rules) plus the Lovable design-prototype preview link — verbatim in [Redesign plan](REDESIGN_PLAN.md). The document's "blank front-end template" scope note describes the prototype project it was written for, not this repository; the roadmap entry records that no implementation scope is accepted and any future work must be reconciled with the existing verified pages and D021. Documentation-only change; no application source, tests, or coverage claimed.

2026-09-12: E09.22 submitted on branch `task/e09-22-tactile-gallery` to implement the user's Tactile Document Gallery plan (P04/[Redesign plan](REDESIGN_PLAN.md)) for the authenticated application: gallery shell with drawer sidebar (sections, storage, sign-out), library top bar with search/refresh/upload toggle, tab/sort view bar, tactile cards, locked-palette and Sora/Manrope design tokens with reduced-motion handling, self-hosted fonts via pinned @fontsource-variable packages, and Fillable page metadata. Library quota display moved into the sidebar; search/sort are client-side over the unchanged cursor API. [Library](LIBRARY.md) and [Product](PRODUCT.md) record the updated contracts. Validation before submission: frontend 322 unit tests pass; measured raw coverage 96.7% lines (1818/1880) and 92.7% branches (2032/2192) through the provenance gate; ESLint, catalog checks and the TypeScript build clean. The production-style browser suite (60 cases, desktop+mobile, real gateway, isolated stack and accounts) finished 59 passed / 1 failed locally: the single failure (session-recovery on mobile) was fixed with a closeNavigation helper and verified green in a targeted rerun; required CI reruns the full suite as the merge gate. No backend source changes.

2026-09-12: E09.23 submitted on branch `task/e09-23-sidebar-only-library` from the user's feedback on the deployed gallery (duplicate switchers, blank line on top): the library tab list is removed in favor of the sidebar sections as the only switcher (aria-current marking, controlled `tab` prop unchanged), and `.authentication:has(.app-shell)` drops the wrapper margin above the shell. Documentation-only impact in [Library](LIBRARY.md) and [Product](PRODUCT.md); no backend changes.

2026-09-12: E09.24 submitted on branch `task/e09-24-full-token-sweep` from the user's request to update the sign-in page and sweep missed surfaces: `authentication.css` rewritten in the gallery card style (surface card, tokenized inputs with focus ring, danger feedback, heading font), and workspace/sidebar/review/fill/profile stylesheets mapped to the semantic tokens with derived success/warning tokens added in base.css. The version badge keeps its exact D022 colors. Validation: 322 unit tests, coverage gate (96.7% lines / 92.7% branches), lint/catalog/TypeScript clean, and authentication/profile/smoke browser cases green on an isolated production-style stack with desktop/mobile screenshots of the sign-in, profile, workspace, and library inspected. No backend changes.
