...
| E09 | Post-release user-requested improvements and automation | E08 | in_progress: E09.1–E09.19 sections below |
...
Local Docker verification: frontend ESLint, catalog/copy checks and the TypeScript production build passed in the pinned image; the full Vitest suite passed 293 tests across 47 files with raw source-bound coverage of 1721/1774 lines (97.01%) and 1959/2108 branches (92.93%), accepted by the independent gate. Tests cover the print-only frame content (layout rules carried over, page-break markers stripped, no application chrome), unavailable-print false paths, the clean print without a redundant revision, save-then-print against the newly saved revision, the abort on a failed pre-save, and the localized local-save suggestion beside the existing download action. The native print dialog itself cannot be exercised in component/browser automation and was verified manually in Chromium on the development stack. Backend source is unchanged; required CI reruns all suites and gates.
