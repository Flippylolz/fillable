...
dev exec -T api python -m app.accounts.cli provision --email previews@example.test --display-name "Превʼю документів" --language uk --password-stdin < fixtures/auth/browser-password.txt
dev exec -T api python -m app.accounts.cli provision --email print@example.test --display-name "Перевірка друку" --language uk --password-stdin < fixtures/auth/browser-password.txt
