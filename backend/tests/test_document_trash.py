...
def used(owner):
    with database().connect() as connection:
        return connection.execute(select(accounts.c.used_bytes).where(accounts.c.user_id == owner.id)).scalar_one()
...
def test_trash_restore_preserves_bytes_versions_deadline_and_owner_isolation():
    ...
    assert web.post(endpoint + '/trash', headers={'X-CSRF-Token': 'bad'}).status_code == 403
    account_service.provision(AccountInput(login='peer', display_name='Peer', password=PASSWORD))
    peer = browser()
    ...
    assert web.post(endpoint + '/trash').status_code == 200
    assert web.get('/api/documents', params={'kind': 'trash'}).json()['items'][0]['purge_after'] == deadline
    ...
    with database().connect() as connection:
        row = connection.execute(select(resources).where(resources.c.id == identity)).mappings().one()
        assert row['purge_after'] - row['trashed_at'] == timedelta(days=30)
        assert connection.execute(select(versions.c.document_model).where(versions.c.document_id == identity)).scalar_one()
...
def test_expiry_and_empty_trash_retain_charges_on_failed_unlink(monkeypatch):
    ...
    with database().begin() as connection:
        connection.execute(update(resources).where(resources.c.id == UUID(saved['id'])).values(purge_after=func.now()))
    assert web.post(endpoint + '/untrash').status_code == 409
    with monkeypatch.context() as patch:
        patch.setattr(FileSystem, 'unlink', lambda *args: (_ for _ in ()).throw(OSError('busy')))
        assert web.delete('/api/documents/trash').json() == {'status': 'pending'}
        assert used(owner) == 2 * len(DATA)
        assert web.get('/api/documents', params={'kind': 'trash'}).json()['items'][0]['deletion_pending']
    ...
    with database().begin() as connection:
        connection.execute(update(resources).where(resources.c.id == UUID(kept['id'])).values(purge_after=func.now()))
